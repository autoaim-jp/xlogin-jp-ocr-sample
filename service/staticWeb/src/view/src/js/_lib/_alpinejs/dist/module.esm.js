...
function initInterceptors2(data2) {
  const isObject2 = (val) => { return typeof val === 'object' && !Array.isArray(val) && val !== null }
  const recurse = (obj, basePath = '') => {
    Object.entries(Object.getOwnPropertyDescriptors(obj)).forEach(([key, { value, enumerable }]) => {
      ...
      } else if (isObject2(value) && value !== obj && !(value instanceof Element)) {
        recurse(value, path)
      }
    })
  }
...
function getElementBoundUtilities(el) {
  const cleanups = []
  const cleanup2 = (callback) => { return cleanups.push(callback) }
...
const hyphenate = cacheStringFunction((str) => { return str.replace(hyphenateRE, '-$1').toLowerCase() })
...
const builtInSymbols = new Set(Object.getOwnPropertyNames(Symbol).map((key) => { return Symbol[key] }).filter(isSymbol))
...
function createGetter(isReadonly = false, shallow = false) {
  return function get3(target, key, receiver) {
    if (key === '__v_isReactive') {
      ...
      return isReadonly
    } if (key === '__v_raw' && receiver === (isReadonly ? shallow ? shallowReadonlyMap : readonlyMap : shallow ? shallowReactiveMap : reactiveMap).get(target)) {
      return target
    }
...
const toReactive = (value) => { return (isObject(value) ? reactive2(value) : value) }
...
magic('watch', (el, { evaluateLater: evaluateLater2, cleanup: cleanup2 }) => {
  return (key, callback) => {
...
function loop(el, iteratorNames, evaluateItems, evaluateKey) {
  const isObject2 = (i) => { return typeof i === 'object' && !Array.isArray(i) }
  const templateEl = el
  evaluateItems((items) => {
...
handler3.inline = (el, { expression }, { cleanup: cleanup2 }) => {
  ...
  cleanup2(() => { return delete root._x_refs[expression] })
}
...
directive('if', (el, { expression }, { effect: effect3, cleanup: cleanup2 }) => {
  ...
  cleanup2(() => { return el._x_undoIf && el._x_undoIf() })
})
...
directive('on', skipDuringClone((el, { value, modifiers, expression }, { cleanup: cleanup2 }) => {
  ...
  cleanup2(() => { return removeListener() })
}))
...
alpine_default.setReactivityEngine({
  reactive: reactive2, effect: effect2, release: stop, raw: toRaw,
})
const src_default = alpine_default

// packages/alpinejs/builds/module.js
const module_default = src_default
export {
